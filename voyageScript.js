	function popUpTeaser()
	{
		//alert("popup");
		$("#teaser").popup();
		$("#teaser").popup("open");
	}
	
	function startVoyage()
	{
		$.mobile.changePage("map.html", { reloadPage : "true"});
	}
	
	function btntest_onclick() 
	{
		//alert("click");
		$.mobile.changePage("atItem.html#checkin");
		/*$("#permaMenu").popup();
		$("#permaMenu").popup("open");*/
	}

	//$(document).ready(function(){
	function createChart(){
		//var w = 960;
		//var h = 600;
		
			var w = $('#chart').width();
			var h = $('#chart').height();
		
		//var w = 300;
		//var h = 300;
		
		var force = d3.layout.force()
				.charge(-3000)
				.linkDistance(60)
				.size([w, h])
		
		var svg = d3.select("#chart").append("svg")
					.attr("viewBox", "0 0 100 100");
		//.attr("class","ui-grid-a .ui-responsive");
		
		var color = d3.scale.category20();
		d3.json(
          'fdgIdent.json',
          function(data) {
		  force
			  .nodes(data.nodes)
			  .links(data.links)
			  .start();

		  var link = svg.selectAll(".link")
			  .data(data.links)
			.enter().append("line")
			  .attr("class", "link")
			  .style("stroke-width", function(d) { return Math.sqrt(d.value); });

		  var node = svg.selectAll(".node")
			  .data(data.nodes)
			.enter().append("g")
			.attr("class", "node")
			.call(force.drag);
			
			/*var ahref = node.append("a")
			            .attr("xlink:href","#teaser")
						.attr("data-role","button")
						.attr("data-rel","popup");*/
		
			
			
			/*node.append("circle")
			  .attr("x",function(d)
			  {
				if(d.group == 1){return -8}//return h/5;}
				if(d.group == 6){return -8;}//return h/50;}
				else return 0;//return h/10;
			  })
			  .attr("y",function(d)
			  {
				if(d.group == 1){return -8}//return h/5;}
				if(d.group == 6){return -8;}//return h/50;}
				else return 0;//return h/10;
			  })
			  .attr("r", function(d)
			  {
				if(d.group == 1){d.fixed = true; return 60;}//return h/5;}
				if(d.group == 6){return 10;}//return h/50;}
				else return 0;//return h/10;
			  })
			  .attr("onclick",function(d)
			  {
				if(d.group == 1)
				{return "return startVoyage()";}
				else 
				{return "return popUpTeaser()";}
			  })
			  .style("fill", function(d) { return color(d.group); });*/
			  
			node.append("image")
			.attr("x",function(d){				
				if(d.group == 1) return -100;
				else if(d.group == 5) return -50;
				else if(d.group == 6) return -12;
			})
			  .attr("y",function(d){				
				if(d.group == 1) return -100;
				else if(d.group == 5) return -50;
				else if(d.group == 6) return -12;
			})
			.attr("xlink:href",function(d){
				if(d.group == 1) {d.fixed = false;}
				return d.image;
				})
			.attr("width",function(d){				
				if(d.group == 1) return 200;
				else if(d.group == 5) return 100;
				else if(d.group == 6) return 25;
			})
			.attr("height",function(d){				
				if(d.group == 1) return 200;
				else if(d.group == 5) return 100;
				else if(d.group == 6) return 25;
			});
			
			/*node.append("text")
			  .attr("dx",-10)
			  .attr("dy",".35em")
			  .attr("class","black")
			  .text(function(d) { return d.name; });*/

		  force.on("tick", function() {
			link.attr("x1", function(d) { return d.source.x; })
				.attr("y1", function(d) { return d.source.y; })
				.attr("x2", function(d) { return d.target.x; })
				.attr("y2", function(d) { return d.target.y; });

				node.attr("transform", function(d) 
				{ 
					//if(d.group == 1) {return "translate(0,0)";}
					//else 
					return "translate(" + d.x + "," + d.y + ")"; 
				});
		  });//force.on()
		});//d3.json()
	}
	//);//ready(function)
	
	function checkinSubmit()
	{
		var ans = $('#answer').val();
		if(ans == 5) 
		{
			$('#nextButton').removeClass('ui-disabled');
			$('#correct').popup();
			$('#correct').popup("open");
		}
		else
		{
			$('#wrong').popup();
			$('#wrong').popup("open");
		}
	}
	
	function showTrivia()
		{
			alert("Hello");
			$.mobile.changePage("atItem.html#trivia");
		}
		/*$(function() {
			$('div[data-role="dialog"]').live('pagebeforeshow', function(e, ui) {
				ui.prevPage.addClass("ui-dialog-background ");
			});
			$('div[data-role="dialog"]').live('pagehide', function(e, ui) {
				$(".ui-dialog-background ").removeClass("ui-dialog-background ");
			});
		});*/